...
const (
	...
	// The UMAC family (RFC 4418). Added in this fork; not present upstream.
	//
	// Every real OpenSSH offers these and lists them ahead of the HMAC
	// variants. Their absence is visible in the first packet of a connection,
	// which for a honeypot whose banner claims OpenSSH is a contradiction that
	// a single nmap --script ssh2-enum-algos will surface.
	UMAC64ETM  = "umac-64-etm@openssh.com"
	UMAC128ETM = "umac-128-etm@openssh.com"
	UMAC64     = "umac-64@openssh.com"
	UMAC128    = "umac-128@openssh.com"

	// Also added in this fork. Upstream carries plain hmac-sha1 but not the
	// encrypt-then-MAC form, which OpenSSH has offered since 6.2 and which is
	// the last entry needed for the MAC list to match a real server exactly.
